...
def plotMaps_pcolormesh(data, figName, values, mapa, lons2d, lats2d, lat, lon, data_points):
  '''
  ...
  vv = []
  if len(lat) != 0:
    for l1, l2, item in zip(lat, lon, data_points):
 #        print(item)
 #        print(stations)
 #        print
      x, y = b(l2, l1)
...
# Land sea mask
mm = Dataset('/pixel/project01/cruman/Data/AIRS/poor_man_mask_crcm5.nc')
  ...
  # Figures for the mean and std of each variable, 
    ...
    if not (var == "t2m" or var == "tt_850" or var == "tt_925"):
      ...
      for index, row in df[df['season'] == per[1]].iterrows():
        ...
        if (svar == 'freq'):
          data_station.append(row[svar]*100)
        else:
